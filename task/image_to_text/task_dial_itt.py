...
async def _put_image() -> Attachment:
    ...
    async with DialBucketClient(api_key=API_KEY, base_url=DIAL_URL) as bucket_client:
        with open(image_path, "rb") as f:
            ...
            attachment = await bucket_client.put_file(name=file_name, mime_type=mime_type_png, content=image_content)
            return Attachment(title=file_name, url=attachment.url, type=mime_type_png)


def start() -> None:
    dalle_client = DialModelClient(endpoint=DIAL_CHAT_COMPLETIONS_ENDPOINT, deployment_name='anthropic.claude-v3-haiku', api_key=API_KEY)
    attachment = asyncio.run(_put_image())

    print(attachment)

    dalle_client.get_completion(role=Role.USER,
